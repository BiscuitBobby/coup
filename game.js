...
function getCardBack() {
  ...
  for (let yy = -40; yy < 760; yy += 56) {
    for (let xx = -40; xx < 552; xx += 56) {
...
function wrapText(ctx, text, cx, cy, maxW, lh) {
  const words = text.split(' '); let line = ''; let y = cy; const lines = [];
...
function drawCrown(x, cx, cy, s, col) {
...
const SEATS = {
  ...
  5: [{ x: 0, z: 3.2 }, { x: -4.3, z: 1.0 }, { x: -2.6, z: -2.6 }, { x: 2.6, z: -2.6 }, { x: 4.3, z: 1.0 }],
  6: [{ x: 0, z: 3.2 }, { x: -3.9, z: 1.6 }, { x: -3.9, z: -1.6 }, { x: 0, z: -3.2 }, { x: 3.9, z: -1.6 }, { x: 3.9, z: 1.6 }],
};
const AI_NAMES = ['Lady Vex', 'Baron Crowe', 'Madame Roux', 'Don Salazar', 'Sir Aldric', 'Lord Daine'];
...
async function resolveBlock(blocker, claim, actor, action) {
  ...
  for (const p of eligible) {
    ...
    if (!p.isHuman) {
      ...
      if (onlineMode && isHost) broadcastState();
      const blockDesc = `blocking ${({ foreign_aid: 'Foreign Aid', assassinate: 'assassination', steal: 'stealing' }[action.type] || action.type)}`;
      wsSendTo(p.id, { type: 'prompt_challenge', claimantName: blocker.name, char: claim, desc: blockDesc });
      const resp = await waitRemote(p.id);
      wants = resp.value;
    } else {
      const blockDesc = `blocking ${({ foreign_aid: 'Foreign Aid', assassinate: 'assassination', steal: 'stealing' }[action.type] || action.type)}`;
      wants = await humanChallengePrompt(blocker, claim, blockDesc);
    }
...
function humanChallengePrompt(claimant, char, desc) {
  hint('');
  return showPrompt(
    `<b>${claimant.name}</b> claims <b>${char}</b>${desc ? ` - ${desc}` : ''}.<br><span style="font-size:16px;color:#b89b66">Do you call it a bluff?</span>`,
...
function showRemoteChallengePrompt(msg) {
  const { claimantName, char, desc } = msg;
  showPrompt(
    `<b>${claimantName}</b> claims <b>${char}</b>${desc ? ` - ${desc}` : ''}.<br><span style="font-size:16px;color:#b89b66">Do you call it a bluff?</span>`,
    [{ label: 'Challenge!', sub: 'risk an influence', cls: 'danger', value: true },
    { label: 'Allow it', sub: 'let it pass', cls: '', value: false }]
